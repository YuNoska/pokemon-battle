...
**ブラッキー／グソクムシャ／ゴリランダー／アシレーヌ／ヒートロトム／サーフゴー**

![ブラッキー](../assets/sprites/umbreon.png) ![グソクムシャ](../assets/sprites/golisopod.png) ![ゴリランダー](../assets/sprites/rillaboom.png) ![アシレーヌ](../assets/sprites/primarina.png) ![ヒートロトム](../assets/sprites/rotom-heat.png) ![サーフゴー](../assets/sprites/gholdengo.png)
...
| サーフゴー | 32 | 0 | 0 | 26 | 8 | 0 | 66 | 特殊の崩し・自己回復 |

## この案を先に勧める理由

ブラッキーのねがいごとで攻撃役を回復し、グソクムシャの物理火力とサーフゴーの特殊火力を両立します。**ねがいごと・じこさいせいの2匹＋きゅうけつの補助回復**。サーフゴーを残すので、回復役を増やした結果、相手を倒せなくなる問題を抑える狙いです。6匹での実戦勝率は未検証です。

## 選出と動かし方

| 相手・目的 | 選出候補3匹 | 方針 |
| --- | --- | --- |
| ブラッキーで支える基本形 | ブラッキー＋グソクムシャ＋ヒートロトム | 炎はロトムで受ける場面を作り、グソクにねがいごとを渡す。ロトムは水技を受けない |
| 水・ドラゴンが多い | ブラッキー＋グソクムシャ＋アシレーヌ | 水・フェアリーの特殊打点も残す。相手の高速水が重ければブラッキーをゴリランダーに替える |
| 前に負けた6匹 | ゴリランダー＋ヒートロトム＋アシレーヌ | インテレオンへ草、グソク・草へ炎、マンダへ妖。ブラッキーやメガ枠を毎回選ぶ必要はない |
| 受け・変化技が多い | ブラッキー＋サーフゴー＋ヒートロトム | ねがいごとで消耗を補い、わるだくみ・トリックで崩す。相手の悪・地面技を見て決める |

例：ブラッキーが安全にねがいごと→次ターン、受けられる攻撃にグソクを交代出し→ターン終了時に回復→攻撃。交代先が倒される攻撃なら、この流れは使わないでください。

### 苦手と戻す条件

電気無効・ステロ・設置除去がありません。ふうせんサーフゴーも一度攻撃を受けると地面無効を失います。ルカリオZを外すため、ブラッキー・ブリジュラスを特殊格闘で一気に崩す手段も失います。アシレーヌの特殊技、サーフゴーの積み、ロトムのトリックを使い分けます。ボルチェンやタスキで負け続けるなら案B、速い特殊エースが必要なら案Cへ。
