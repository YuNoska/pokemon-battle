# メガルカリオZ構築 — ブラッキーで支える攻撃サイクル

> 2026-09-19にユーザーが構築する対象として選択。以前の案Cを、このページで継続管理します。
...
今回はメガルカリオZを主役にする構築です。以前のv2.0のサーフゴーをブラッキーへ替え、ねがいごとでルカリオZなどを支えます。**ねがいごと・はねやすめの2匹**。ルカリオZの高速特殊格闘とマンダの物理火力を相手に合わせて選びます。メガ2枠を登録しますが、基本はどちらか1匹を選出します。

## まず作る・使う順番

1. **特殊型ルカリオを用意**。通常のルカリオナイトではなく**ルカリオナイトZ**。おくびょう・H2/C32/S32に合わせ、はどうだん／ラスターカノン／あくのはどう／わるだくみを設定。
2. **ゴリランダー＋アシレーヌ**を先に合わせる。この3匹を基本選出の出発点にする。草フィールドはルカリオが受けるじしんを弱め、水・地面への攻撃もゴリランダーで補う。
3. **ブラッキー**で回復支援。ただし「ねがいごと→ルカリオ交代」は交代先がそのターンの攻撃を耐えられるときだけ。毎試合ブラッキーを選ぶ必要はない。
4. **ロトム**は炎打点とスカーフ。写真の個体のおにびをトリックへ。**マンダ**はルカリオで崩しにくい相手への別メガ枠として用意。

6匹の種族は写真で確認済みですが、育成・技・特性・メガストーンまで揃っているとは断定していません。上の表を入力用の正本にしてください。

### ルカリオZで勝つ流れ

- ゴリランダーのとんぼ、アシレーヌのクイックターン、ロトムのボルチェンで有利対面を作る。遅い交代技は使う本人が耐える必要があり、速い交代技では着地したルカリオが被弾する。
- **攻撃で倒せるなら、わるだくみを挟まず攻撃**。格闘は鋼・悪、鋼は妖、悪はゴースト等への打点として使い分ける。
- 積むのは相手が交代する局面や、被ダメージを確認して行動が残る場面。はどうのぼうごは接触技を軽減する特性で、特殊技・じしんまで受けられるわけではない。
- 最速設定でも相手のスカーフ・速度上昇・先制技を無視しない。タスキ・がんじょうの相手は一撃で倒れる前提にしない。

## 選出と動かし方

| 相手・目的 | 選出候補3匹 | 方針 |
| --- | --- | --- |
| 基本選出 | ルカリオZ＋ゴリランダー＋アシレーヌ | 草・水・妖で補完し、ルカリオの特殊技を通す |
...
- [Serebii：ルカリオの考察](https://www.serebii.net/potw-champions/448.shtml)：2026-09-19に再確認。シングル欄でルカリオZの相方としてゴリランダー・アシレーヌを推奨。あくのはどうも選択肢として記載。記事の配分はH5/B4/C32/S25ですが、この案では最速のH2/C32/S32を採用しています。記事の耐久計算をこの配分へ流用しません。
- [バトルデータベース：ブラッキー・M-6シングル](https://champs.pokedb.tokyo/pokemon/show/0197-00?season=6&rule=0)：9/18に確認したねがいごと・まもる型の採用傾向。9/19に再集計した数値ではありません。
- [既存の速度・火力概算](../reports/2026-09-18-damage-checks.md)：同じ設定のルカリオZ・ゴリランダー・ロトムの参考。相手の配分・持ち物で結果は変わります。

ブラッキー・別メガのマンダ・ロトムを含む6匹の組み合わせは、回復と交代を好む希望に合わせた独自案です。海外の完成構築の転載や、実戦で勝率を確認した構築ではありません。
